...
/**
 * Form component for the "Umiejętności" custom section.
 * Manages a list of skill categories, each with a label and comma-separated skill list.
 */
      ...
      {section.items.map((item, itemIndex) => (
        <div key={item.id} className="bg-white p-[15px] rounded-md mb-3 border border-gray-300">
          ...
          <div className="mb-[15px]">
            <label className="block mb-1.5 text-gray-900 font-medium text-sm">Kategoria:</label>
            <input
              type="text"
              className="form-input w-full"
              value={item.data.category as string}
              onChange={(e) => onUpdateItem(section.id, item.id, 'category', e.target.value)}
              placeholder="np. Programowanie, Design, Języki"
            />
          </div>
          <div className="mb-[15px]">
            <label className="block mb-1.5 text-gray-900 font-medium text-sm">Umiejętności:</label>
            <textarea
              className="form-input w-full resize-none"
              value={item.data.skills as string}
              onChange={(e) => onUpdateItem(section.id, item.id, 'skills', e.target.value)}
