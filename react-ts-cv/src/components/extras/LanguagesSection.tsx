...
/**
 * Form component for the "Języki obce" custom section.
 * Manages a list of language proficiency entries with language name and level.
 */
      ...
      {section.items.map((item, itemIndex) => (
        <div key={item.id} className="bg-white p-[15px] rounded-md mb-3 border border-gray-300">
          ...
          <div className="mb-[15px]">
            <label className="block mb-1.5 text-gray-900 font-medium text-sm">Język:</label>
            <input
              type="text"
              className="form-input w-full"
              value={item.data.language as string}
              onChange={(e) => onUpdateItem(section.id, item.id, 'language', e.target.value)}
              placeholder="np. Angielski"
            />
          </div>
          <div className="mb-[15px]">
            <label className="block mb-1.5 text-gray-900 font-medium text-sm">Poziom:</label>
            <input
              type="text"
              className="form-input w-full"
              value={item.data.level as string}
              onChange={(e) => onUpdateItem(section.id, item.id, 'level', e.target.value)}
